...
Many healthchecks use a command like `["CMD", "curl", "-f", "http://localhost:3000/health"]`, where the `-f` flag means "fail fast with no output at all on server errors" (source: curl's man page). Any valid command could be used though. Some projects include a script for checking health, such as [Beszel](https://www.beszel.dev/): `["CMD", "/beszel", "health", "--url", "http://localhost:8090"]`.
